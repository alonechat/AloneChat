...
Usage:
    from AloneChat.logging import get_logger
...
Configuration:
    from AloneChat.logging import configure_logging, LogConfig
...
class LoggingManager:
    ...
    def configure(self, config: LogConfig) -> None:
        ...
        # Resolve root level
        root_level = self._resolve_level(config.level)

        # Resolve component-specific levels
        component_levels = {}
        for component, level_str in config.component_levels.items():
            component_levels[component] = self._resolve_level(level_str)

        # Create log directory if file output is enabled
        if config.file_output:
            Path(config.log_dir).mkdir(parents=True, exist_ok=True, mode=0o750)
        ...
        for component, level_val in component_levels.items():
            component_logger = logging.getLogger(component)
            component_logger.setLevel(level_val)
    ...
    def _resolve_level(level_str: str) -> int:
        try:
            ...
            valid = ', '.join(
                ...
                if name.isupper() and name not in ('NOTSET', 'FATAL', 'WARN')
            )
            raise ValueError(
                f"Invalid log level: '{level_str}'. Valid levels: {valid}"
            ) from None
