...
class AloneChatAPIClient:
    ...
    # -------------------- WeChat-like social features --------------------
    async def list_users(self, q: str = "", limit: int = 50) -> Dict[str, Any]:
        return await self._make_request(f"/api/users?q={q}&limit={int(limit)}")

    async def list_friends(self) -> Dict[str, Any]:
        return await self._make_request("/api/friends")

    async def send_friend_request(self, to_username: str, message: str = "") -> Dict[str, Any]:
        return await self._make_request(
            "/api/friends/request",
            method="POST",
            data={"to_username": to_username, "message": message},
        )

    async def incoming_friend_requests(self) -> Dict[str, Any]:
        return await self._make_request("/api/friends/requests/incoming")

    async def outgoing_friend_requests(self) -> Dict[str, Any]:
        return await self._make_request("/api/friends/requests/outgoing")

    async def accept_friend_request(self, request_id: int) -> Dict[str, Any]:
        return await self._make_request(f"/api/friends/requests/{int(request_id)}/accept", method="POST")

    async def reject_friend_request(self, request_id: int) -> Dict[str, Any]:
        return await self._make_request(f"/api/friends/requests/{int(request_id)}/reject", method="POST")

    async def list_conversations(self, limit: int = 50) -> Dict[str, Any]:
        return await self._make_request(f"/api/conversations?limit={int(limit)}")

    async def update_conversation_settings(self, with_user: str, pinned: bool | None = None, muted: bool | None = None) -> Dict[str, Any]:
        payload: Dict[str, Any] = {}
        if pinned is not None:
            payload["pinned"] = bool(pinned)
        if muted is not None:
            payload["muted"] = bool(muted)
        return await self._make_request(f"/api/conversations/{with_user}/settings", method="POST", data=payload)

    async def get_history(self, with_user: str, limit: int = 50) -> Dict[str, Any]:
        return await self._make_request(
            f"/api/messages/history?with_user={with_user}&limit={int(limit)}"
        )
