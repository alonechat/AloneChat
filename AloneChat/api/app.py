...
async def send_private_message(msg_req: PrivateMessageRequest, request: Request):
    ...
    # Friend-gate private chats
    if not get_friend_service().are_friends(sender, msg_req.recipient):
...
async def create_friend_request(req: FriendRequestCreate, request: Request):
    current_user = _get_user(request)
    to_user = (req.to_user or "").strip()
    if not to_user:
        raise HTTPException(status_code=400, detail="Missing to_user")
    result = get_friend_service().send_request(current_user, to_user)
...
async def accept_friend_request(req: FriendRequestDecision, request: Request):
    current_user = _get_user(request)
    from_user = (req.from_user or "").strip()
    if not from_user:
        raise HTTPException(status_code=400, detail="Missing from_user")
    result = get_friend_service().accept_request(current_user, from_user)
    if not result.get("success"):
        raise HTTPException(status_code=400, detail=result.get("error", "Failed"))
    return result
...
async def reject_friend_request(req: FriendRequestDecision, request: Request):
    current_user = _get_user(request)
    from_user = (req.from_user or "").strip()
    if not from_user:
        raise HTTPException(status_code=400, detail="Missing from_user")
    result = get_friend_service().reject_request(current_user, from_user)
    if not result.get("success"):
        raise HTTPException(status_code=400, detail=result.get("error", "Failed"))
    return result
...
async def list_friends(request: Request):
    current_user = _get_user(request)
    friends = get_friend_service().list_friends(current_user)
    return {"success": True, "friends": friends, "count": len(friends)}


@app.get("/api/friends/requests")
async def list_friend_requests(request: Request, direction: str = 'incoming', limit: int = 100):
    current_user = _get_user(request)
    direction = (direction or 'incoming').lower()
    if direction not in ('incoming', 'outgoing'):
        raise HTTPException(status_code=400, detail="Invalid direction")
    reqs = get_friend_service().list_requests(current_user, direction=direction, limit=limit)
...
async def send_message(request: Request):
    ...
    if target:
        # Friend-gate private chats
        if not get_friend_service().are_friends(username, target):
            raise HTTPException(status_code=403, detail="NOT_FRIENDS")
