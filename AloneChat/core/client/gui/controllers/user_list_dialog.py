...
class UserListDialog:
    ...
    def __init__(
        ...
        # Friend-gated private chat
        is_friend: Optional[Callable[[str], bool]] = None,
        on_add_friend: Optional[Callable[[str], None]] = None,
    ):
        self.root = root
    ...
    def show(self) -> None:
        ...
        ttk.Label(frm, text="Double-click a friend to chat. Double-click others to add.").pack(anchor="w", pady=(0, 8))
    ...
    def _on_select(self) -> None:
        ...
        if 0 <= idx < len(self.users_data):
            ...
            # Friend-gated: only friends can open private chat.
            is_friend = self.is_friend(user_id) if self.is_friend else True
            if is_friend:
                self.on_select_user(user_id)
                self.hide()
                return

            # Not a friend -> simplify flow: confirm and send request.
            if not self.on_add_friend:
                messagebox.showinfo("Friends", "You can only chat with friends. Please add them first.")
                return

            ok = messagebox.askyesno("Add friend", f"{user_id} is not your friend yet. Send a friend request?")
            if ok:
                self.on_add_friend(user_id)
