...
class APIClient:
    ...
    # Friend system API
    # -----------------
    async def create_friend_request(self, to_user: str) -> Dict[str, Any]:
        return await self._request("POST", "/api/friends/request", {"to_user": to_user})

    async def accept_friend_request(self, from_user: str) -> Dict[str, Any]:
        return await self._request("POST", "/api/friends/accept", {"from_user": from_user})

    async def reject_friend_request(self, from_user: str) -> Dict[str, Any]:
        return await self._request("POST", "/api/friends/reject", {"from_user": from_user})

    async def list_friends(self) -> Dict[str, Any]:
        return await self._request("GET", "/api/friends/list")

    async def list_friend_requests(self, direction: str = 'incoming', limit: int = 100) -> Dict[str, Any]:
        return await self._request("GET", "/api/friends/requests", params={"direction": direction, "limit": limit})
    
    
    # -------- Backward-compatible aliases (GUI expects these names) --------
    async def friends_list(self) -> Dict[str, Any]:
        """Alias for list_friends()."""
        return await self.list_friends()

    async def friends_request(self, to_user: str) -> Dict[str, Any]:
        """Alias for create_friend_request()."""
        return await self.create_friend_request(to_user)

    async def friends_accept(self, from_user: str) -> Dict[str, Any]:
        """Alias for accept_friend_request()."""
        return await self.accept_friend_request(from_user)

    async def friends_reject(self, from_user: str) -> Dict[str, Any]:
        """Alias for reject_friend_request()."""
        return await self.reject_friend_request(from_user)

    async def friends_requests(self, direction: str = 'incoming', limit: int = 100) -> Dict[str, Any]:
        """Alias for list_friend_requests()."""
        return await self.list_friend_requests(direction=direction, limit=limit)
