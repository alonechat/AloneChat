...
The upstream project targets newer versions of ``websockets`` which expose
``websockets.asyncio.server.ServerConnection``.

Some environments (and CI images) ship an older ``websockets`` package where
that import path does not exist. To keep the project runnable across a wider
range of versions, we import a compatible protocol type from whichever module
is available.
"""
...
try:  # websockets >= 12
    from websockets.asyncio.server import ServerConnection  # type: ignore
except Exception:  # pragma: no cover
    try:  # websockets 10/11
        from websockets.server import WebSocketServerProtocol as ServerConnection  # type: ignore
    except Exception:  # pragma: no cover
        # websockets legacy fallback
        from websockets.legacy.server import WebSocketServerProtocol as ServerConnection  # type: ignore
